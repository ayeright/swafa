...
from swafa.new_callbacks import FactorAnalysisVariationalInferenceCallbackFixD
...
class Objective:
    ...
    precision of the prior of the posterior, sigma value of PPCA variational distribution and the precision of the additive noise distribution of the targets.
    ...
    Args:
        ...
        fixed_d_range: The minimum and maximum values of the hyperparameter range of the sigma value of PPCA 
    ...
    def __call__(self, trial: optuna.Trial):
        ...
        cv_mll, _ = self.cross_validate(learning_rate, prior_precision, noise_precision, fixed_d)

        return cv_mll

    def cross_validate(self, learning_rate: float, prior_precision: float, noise_precision: float, fixed_d: float) -> (float, float):
        """
        ...
        for train_index, test_index in self.k_fold.split(self.dataset):
            mll, rmse = self.train_and_test(train_index, test_index, learning_rate, prior_precision, noise_precision, fixed_d)
            mll_list.append(mll)
    ...
    def train_and_test(self, train_index: np.ndarray, test_index: np.ndarray, learning_rate: float,
                       prior_precision: float, noise_precision: float, fixed_d: float) -> (float, float):
        """
        ...
        model, variational_callback = self.train_model(
            X_train, y_train, learning_rate, prior_precision, standardised_noise_precision, fixed_d, 
        )
    ...
    def train_model(self, X: Tensor, y: Tensor, learning_rate: float, prior_precision: float, noise_precision: float, fixed_d: float, 
                    ) -> (FeedForwardGaussianNet, FactorAnalysisVariationalInferenceCallbackFixD):
        """
        ...
        variational_callback = FactorAnalysisVariationalInferenceCallbackFixD(
        ...
        # where the logger is stored
        tb_logger = pl_loggers.TensorBoardLogger(save_dir=f"/home/s2174572/BDL/Bayesian_DL/larger_NN_experiments/new_pl_log/neural_net_predictions_fixD_Gaussian/{self.log_path}") # Check bugs
    ...
    def sample_predictions(self, model: FeedForwardGaussianNet,
                           variational_callback: FactorAnalysisVariationalInferenceCallbackFixD, X: Tensor, y_mean: float,
    ...
    def predict(self, model: FeedForwardGaussianNet, variational_callback: FactorAnalysisVariationalInferenceCallbackFixD,
...
def run_experiment(
    ...
    The hyperparameters which are tuned are the learning rate with which to update the parameters of the posterior, the
    precision of the prior of the posterior, the sigma of PPCA variational distribution and the precision of the additive noise distribution of the targets.
    Hyperparameter values are sampled from log-uniform distributions. The user must define the hyperparameter ranges.
...
def run_trial(
    ...
    The hyperparameters which are tuned are the learning rate with which to update the parameters of the posterior, the
    precision of the prior of the posterior, the sigma value of PPCA variational distribution and the precision of the additive noise distribution of the targets.
    Hyperparameter values are sampled from log-uniform distributions. The user must define the hyperparameter ranges.
    ...
    val_mll, val_rmse = objective.cross_validate(learning_rate, prior_precision, noise_precision, fixed_d)
    ...
    test_mll, test_rmse = objective.train_and_test(
        train_index, test_index, learning_rate, prior_precision, noise_precision, fixed_d, 
    )
...
def main(dataset_label: str, dataset_input_path: str, results_output_dir: str):
    ...
    """
    with open("/home/s2174572/BDL/Bayesian_DL/params_new_v2.yaml", 'r') as fd: # v2 refers to fixD; new refers to larger NN
