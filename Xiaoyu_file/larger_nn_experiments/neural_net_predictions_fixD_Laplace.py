...
from swafa.new_callbacks import FactorAnalysisVariationalInferenceCallbackFixDLaplace
...
class Objective:
    ...
    data is used to approximate the posterior distribution of the weights of the neural network via the VIPPCA algorithm.
    ...
    alpha of the prior of the posterior, the sigma value of the PPCA variational distribution and the precision of the additive noise distribution of the targets.
    ...
    Args:
        ...
        prior_alpha_range: The minimum and maximum values of the hyperparameter range of the alpha of the prior
            of the posterior.
        ...
        fixed_d_range: The minimum and maximum values of the hyperparameter range of the sigma value of the PPCA 
    ...
    def __init__(
            ...
            prior_alpha_range: List[float],
            noise_precision_range: List[float],
            fixed_d_range: List[float],
            n_bma_samples: int,
        ...
        self.prior_alpha_range = prior_alpha_range
        self.noise_precision_range = noise_precision_range
        self.fixed_d_range = fixed_d_range
    ...
    def __call__(self, trial: optuna.Trial):
        ...
        prior_alpha = trial.suggest_loguniform('prior_alpha', *self.prior_alpha_range)
        noise_precision = trial.suggest_loguniform('noise_precision', *self.noise_precision_range)
        fixed_d = trial.suggest_loguniform('fixed_d',*self.fixed_d_range)

        cv_mll, _ = self.cross_validate(learning_rate, prior_alpha, noise_precision, fixed_d)

        return cv_mll

    def cross_validate(self, learning_rate: float, prior_alpha: float, noise_precision: float, fixed_d: float) -> (float, float):
        """
        ...
        via the VIPPCA algorithm. Then use the posterior to compute metrics for the test data.

        Args:
            learning_rate: The learning rate with which to update the parameters of the VIPPCA callback.
            prior_alpha: The alpha of the prior of the posterior.
            noise_precision: The precision of the additive noise distribution of the targets.
            fixed_d: The fixed value of the diagonal of the covariance matrix of posterior distribution.
        ...
        for train_index, test_index in self.k_fold.split(self.dataset):
            mll, rmse = self.train_and_test(train_index, test_index, learning_rate, prior_alpha, noise_precision, fixed_d)
            mll_list.append(mll)
    ...
    def train_and_test(self, train_index: np.ndarray, test_index: np.ndarray, learning_rate: float,
                       prior_alpha: float, noise_precision: float, fixed_d: float) -> (float, float):
        """
        Train and test a neural network for the given train and test indices and hyperparameters.

        Using the training data, approximate the posterior distribution of the weights of a neural network via the VIPPCA
        ...
        Args:
            ...
            prior_alpha: The alpha of the prior of the posterior.
            noise_precision: The precision of the additive noise distribution of the targets.
            fixed_d: The fixed value of the diagonal of the covariance matrix of the posterior distribution.
        ...
        model, variational_callback = self.train_model(
            X_train, y_train, learning_rate, prior_alpha, standardised_noise_precision, fixed_d, 
        )
    ...
    def train_model(self, X: Tensor, y: Tensor, learning_rate: float, prior_alpha: float, noise_precision: float, fixed_d: float, 
                    ) -> (FeedForwardGaussianNet, FactorAnalysisVariationalInferenceCallbackFixDLaplace):
        """
        Given the input data, approximate the posterior distribution of the weights of a neural network via the VIPPCA
        algorithm.
        ...
        variational_callback = FactorAnalysisVariationalInferenceCallbackFixDLaplace(
        ...
        tb_logger = pl_loggers.TensorBoardLogger(save_dir=f"/home/s2174572/BDL/Bayesian_DL/larger_NN_experiments/new_pl_log/neural_net_predictions_fixD_Laplace/{self.log_path}") # Check bugs
    ...
    def sample_predictions(self, model: FeedForwardGaussianNet,
                           variational_callback: FactorAnalysisVariationalInferenceCallbackFixDLaplace, X: Tensor, y_mean: float,
    ...
    def predict(self, model: FeedForwardGaussianNet, variational_callback: FactorAnalysisVariationalInferenceCallbackFixDLaplace,
...
def run_experiment(
    ...
    posterior distribution of the weights of a neural network trained via the VIPPCA algorithm. Optionally, use the best
    ...
    Args:
        ...
        noise_precision_range: The minimum and maximum values of the hyperparameter range of the precision of the
            additive noise distribution of the targets.
        fixed_d_range: The minimum and maximum values of the hyperparameter range of the fixed_D value in diagonal of the matrix.
        n_bma_samples: The number of samples in each Bayesian model averaging when testing.
        hidden_dims: The dimension of each hidden layer in the neural network. hidden_dims[i] is the dimension of the
            i-th hidden layer. If None, the input will be connected directly to the output.
        hidden_activation_fn: The activation function to apply to the output of each hidden layer. If None, will be set
    ...
    for i, train_test in enumerate(train_test_indices):
        ...
        trial_results = run_trial(
            ...
            prior_alpha_range=prior_alpha_range,
            noise_precision_range=noise_precision_range,
            fixed_d_range = fixed_d_range,
...
def run_trial(
    ...
    Args:
        ...
        noise_precision_range: The minimum and maximum values of the hyperparameter range of the precision of the
            additive noise distribution of the targets.
        fixed_d_range: The minimum and maximum values of the hyperparameter range of the fixed diagonal value of the 
    ...
    prior_alpha = study.best_params['prior_alpha']
    noise_precision = study.best_params['noise_precision']
    fixed_d = study.best_params['fixed_d']

    val_mll, val_rmse = objective.cross_validate(learning_rate, prior_alpha, noise_precision, fixed_d)
    ...
    test_mll, test_rmse = objective.train_and_test(
        train_index, test_index, learning_rate, prior_alpha, noise_precision, fixed_d, 
    )
...
def main(dataset_label: str, dataset_input_path: str, results_output_dir: str):
    ...
    """
    with open("/home/s2174572/BDL/Bayesian_DL/params_new_v2_laplace.yaml", 'r') as fd: # new refers to larger NN; v2 refer to fixD; laplace means a different prior alpha range
    ...
    results, agg_results = run_experiment(
        ...
        prior_alpha_range=dataset_params['prior_alpha_range'],
        noise_precision_range=dataset_params['noise_precision_range'],
        fixed_d_range = dataset_params['fixed_d_range'],
