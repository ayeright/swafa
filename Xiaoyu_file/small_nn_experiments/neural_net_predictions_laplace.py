...
from swafa.new_callbacks import FactorAnalysisVariationalInferenceCallbackLaplace
...
class Objective:
    ...
    alpha of the prior of the posterior and the precision of the additive noise distribution of the targets.
    ...
    Args:
        ...
        prior_alpha_range: The minimum and maximum values of the hyperparameter range of the alpha value of the prior
            of the posterior.
    ...
    def __call__(self, trial: optuna.Trial):
        ...
        cv_mll, _ = self.cross_validate(learning_rate, prior_alpha, noise_precision)

        return cv_mll

    def cross_validate(self, learning_rate: float, prior_alpha: float, noise_precision: float) -> (float, float):
        """
        ...
        Args:
            ...
            prior_alpha: The alpha value of the prior of the posterior.
        ...
        for train_index, test_index in self.k_fold.split(self.dataset):
            mll, rmse = self.train_and_test(train_index, test_index, learning_rate, prior_alpha, noise_precision)
            mll_list.append(mll)
    ...
    def train_and_test(self, train_index: np.ndarray, test_index: np.ndarray, learning_rate: float,
                       prior_alpha: float, noise_precision: float) -> (float, float):
        """
        ...
        model, variational_callback = self.train_model(
            X_train, y_train, learning_rate, prior_alpha, standardised_noise_precision,
        )
    ...
    def train_model(self, X: Tensor, y: Tensor, learning_rate: float, prior_alpha: float, noise_precision: float,
                    ) -> (FeedForwardGaussianNet, FactorAnalysisVariationalInferenceCallbackLaplace):
        """
        ...
        variational_callback = FactorAnalysisVariationalInferenceCallbackLaplace(
        ...
        tb_logger = pl_loggers.TensorBoardLogger(save_dir=f"/home/s2174572/BDL/Bayesian_DL/pl_logs/neural_network_predictions/{self.log_path}") # Check bugs
    ...
    def sample_predictions(self, model: FeedForwardGaussianNet,
                           variational_callback: FactorAnalysisVariationalInferenceCallbackLaplace, X: Tensor, y_mean: float,
    ...
    def predict(self, model: FeedForwardGaussianNet, variational_callback: FactorAnalysisVariationalInferenceCallbackLaplace,
...
def run_trial(
    ...
    val_mll, val_rmse = objective.cross_validate(learning_rate, prior_alpha, noise_precision)
    ...
    test_mll, test_rmse = objective.train_and_test(
        train_index, test_index, learning_rate, prior_alpha, noise_precision,
    )
...
def main(dataset_label: str, dataset_input_path: str, results_output_dir: str):
    ...
    """
    # NOTE: params_new refers to larger model
    with open("/home/s2174572/BDL/Bayesian_DL/params_laplace.yaml", 'r') as fd:
