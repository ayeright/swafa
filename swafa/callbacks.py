...
class FactorAnalysisVariationalInferenceCallback(Callback):
    ...
    def sample_weight_vector(self) -> Tensor:
        ...
        """
        self._h = torch.normal(torch.zeros(self.latent_dim), torch.ones(self.latent_dim), device=self.device)[:, None]
        self._z = torch.normal(torch.zeros(self.weight_dim), torch.ones(self.weight_dim), device=self.device)[:, None]
