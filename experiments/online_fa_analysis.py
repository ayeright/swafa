...
def run_analysis(results: pd.DataFrame, analysis_output_dir: str, metric_columns: List[str]):
    """
    Aggregate the experiment results and generate plots.

    For each experiment, defined by the parameters observation_dim, latent_dim, spectrum_min and spectrum_max, group by
    n_samples and compute the mean and standard error of the values in each column specified in metric_columns.

    Save these results to csv files. Also, for each experiment plot the means with standard error bars and save to png
    files.
    ...
    for _, params in param_combinations.iterrows():
        ...
        generate_and_save_error_bar_plot(
            group_means, group_standard_errors, os.path.join(analysis_output_dir, f'online_fa__{params_str}.png'),
        )
...
def main(results_input_path: str, analysis_output_dir: str):
    ...
    run_analysis(
        results,
        analysis_output_dir,
        params['online_fa_analysis']['metric_columns'],
    )
