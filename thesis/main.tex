...
\end{equation}
where $L$ is the Monte Carlo average size, each $\mathcal{B}_l$ is a mini-batch of $M$ training examples sampled from the full dataset $\mathcal{D}$ of size $N$, and $\theta_l \sim q(\theta)$. Then 
\begin{equation}\label{eqn:expected_grad_log_likelihood}
	\nabla_{\matr{c}, \matr{F}, \Psi} \E_{q(\theta)} [\log p(\mathcal{D} | \theta)]
...
\end{equation}
Now, in order to obtain these partial derivatives, the re-parameterisation trick \cite{goodfellow2016} can be used. First note that, by re-writing Equation (\ref{eqn:fa_model}), sampling $\theta_l \sim q(\theta) = \mathcal{N}\big(\matr{c}, \matr{FF}^{\intercal} + \Psi\big)$ is equivalent to setting  
\begin{equation}\label{eqn:fa_reparam_trick}
	\theta_l = \matr{Fh} + \matr{c} + \Psi^{1/2} \matr{z}
\end{equation}
from some $\matr{h} \sim \mathcal{N}(\matr{0}^K, \matr{I}^{K \times K})$ and $\matr{z} \sim \mathcal{N}(\matr{0}^D, \matr{I}^{D \times D})$, where the exponent is applied to $\Psi$ element-wise. Then it follows from the chain rule of calculus that
\begin{align}
\begin{split}\label{eqn:chain_rule_log_likelihood}
	 \nabla_{\matr{c}, \matr{F}, \Psi} \Bigg( \frac{1}{M} \sum_{(\matr{x}, y) \in \mathcal{B}_l} \log p(y | \matr{x}, \theta_l) \Bigg)
	 & = \nabla_{\theta_l} \Bigg( \frac{1}{M} \sum_{(\matr{x}, y) \in \mathcal{B}_l} \log p(y | \matr{x}, \theta_l) \Bigg)
	 \nabla_{\matr{c}, \matr{F}, \Psi} \theta_l \\
	 & = \matr{g}_{\theta_l} \nabla_{\matr{c}, \matr{F}, \Psi} \theta_l,
\end{split}
\end{align}
where $\matr{g}_{\theta_l} = \nabla_{\theta_l} \Big( \frac{1}{M} \sum_{(\matr{x}, y) \in \mathcal{B}_l} \log p(y | \matr{x}, \theta_l) \Big)$. Hence, substituting Equation (\ref{eqn:chain_rule_log_likelihood}) into Equation (\ref{eqn:expected_grad_log_likelihood}),
\begin{equation}
	\nabla_{\matr{c}, \matr{F}, \Psi} \E_{q(\theta)} [\log p(\mathcal{D} | \theta)]
...
\begin{equation}
	 ...
	 \approx \frac{1}{L} \sum_{l=1}^{L} N \matr{g}_{\theta_l} \matr{h}^\intercal,
\end{equation}
\begin{equation}\label{eqn:grad_likelihood_wrt_psi}
	 \nabla_\Psi \E_{q(\theta)} [\log p(\mathcal{D} | \theta)]
	 \approx \diag\Big( \diag\Big( \frac{1}{L} \sum_{l=1}^{L} \frac{N}{2} \matr{g}_{\theta_l} \matr{z}^\intercal \Psi^{-1/2} \Big)\Big).
\end{equation}
This is convenient, since $\matr{g}_{\theta_l}$ can be computed efficiently using the back-propagation algorithm \cite{rumelhart1986}. Indeed, deep learning frameworks such as PyTorch \cite{paszke2019} compute the gradient of the average mini-batch negative log-likelihood, $-\matr{g}_{\theta_l}$, as standard. 
...
where 
\begin{equation}
	\matr{B} = \matr{F}^\intercal \matr{A}, 
	\quad \matr{C} = \matr{A} (\matr{I} + \matr{B})^{-1}
	\quad \text{and} \quad \matr{D} = -\matr{A}  + \matr{C} \matr{B}^\intercal.
\end{equation}
Also,
\begin{align}
\begin{split}
...
\begin{algorithm}[!htbp] 
	...
	\begin{algorithmic}[1]
		...
		\State $\matr{D} \leftarrow -\matr{A}  + \matr{C} \matr{B}^\intercal$
		\State $\matr{s} \leftarrow -\frac{1}{2} + \frac{1}{2} \text{sum}\big(\matr{C} \odot \matr{A}, \text{ dim} = 1\big) \odot \psi$
		\For {$t = 1,\dots, T$}
			...
			\State $\hat{\matr{g}}_\theta \leftarrow N \cdot \nabla_\theta \big(-\frac{1}{M} \sum_{(\matr{x}, y) \in \mathcal{B}} \log p(y | \matr{x}, \theta)\big)$ (via back-propagation)
			\State $\hat{\matr{g}}_\matr{c} \leftarrow \hat{\matr{g}}_\matr{c} + \alpha \matr{c} + \hat{\matr{g}}_\theta$
			\State $\hat{\matr{G}}_\matr{F} \leftarrow \hat{\matr{G}}_\matr{F} + \matr{D} + \alpha \matr{F} + \hat{\matr{g}}_\theta \matr{h}^\intercal$
			\State $\hat{\matr{g}}_\gamma \leftarrow \hat{\matr{g}}_\gamma + \matr{s} + \frac{\alpha}{2} \psi + \frac{1}{2} \hat{\matr{g}}_\theta \odot \big(\psi^{1/2} \odot \matr{z}\big)$
			\If {$t \bmod L = 0$}
				\State $\matr{c} \leftarrow \matr{c} - \eta_\matr{c} \big(\frac{1}{L}\hat{\matr{g}}_\matr{c}\big)$
				\State $\matr{F} \leftarrow \matr{F} - \eta_\matr{F} \big(\frac{1}{L}\hat{\matr{G}}_\matr{F}\big)$
				\State $\gamma \leftarrow \gamma - \eta_\gamma \big(\frac{1}{L}\hat{\matr{g}}_\gamma\big)$
				...
				\State Recalculate $\matr{A}, \matr{B}, \matr{C}, \matr{D}$ and $\matr{s}$ according to lines 4-8
			\EndIf
		\EndFor
...
\begin{table}[h!]
	\begin{center}
		\begin{tabular}{|| p{0.21\linewidth} p{0.21\linewidth} p{0.21\linewidth} p{0.21\linewidth} ||} 
			...
			\hline
		\end{tabular}
		\caption{Distances between the true posterior distribution of the parameter vector of a linear regression model and the approximate FA posterior estimated using VIFA. Relative distances between the true and approximate means and covariances are shown. Each relative distance is the Frobenius norm of the difference between the true parameter and the approximate parameter divided by the Frobenius norm of the true parameter. Also shown is the 2-Wasserstein distance between the true posterior Gaussian distribution and the approximate Gaussian distribution. Each distance is the mean value over ten trials with different random seeds, and standard errors are also given.}
...
\begin{table}[h!]
	\begin{center}
		\begin{tabular}{|| p{0.21\linewidth} p{0.21\linewidth} p{0.21\linewidth} p{0.21\linewidth} ||} 
 			\hline
 			Dataset & Relative Distance from Mean & Relative Distance from Covariance & Wasserstein Distance \\ [0.5ex] 
 			\hline\hline
			Energy Efficiency & 0.0042 & 0.0765 & 0.2674 \\ 
			\hline
			Boston Housing & 0.0253 & 0.1467 & 0.1871 \\ 
			\hline
			Concrete Strength & 0.0058 & 0.1267 & 0.1454 \\ 
			\hline
 			Yacht Hydro. & 0.0437 & 0.0477 & 0.4461 \\ [1ex] 
			\hline
		\end{tabular}
		\caption{For each UCI dataset, the distance between the true posterior distribution of the parameter vector of a linear regression model and the approximate FA posterior estimated by VIFA. Relative distances between the true and approximate means and covariances are shown. Each relative distance is the Frobenius norm of the difference between the true parameter and the approximate parameter divided by the Frobenius norm of the true parameter. Also shown is the 2-Wasserstein distance between the true posterior Gaussian distribution and the approximate Gaussian distribution.}
