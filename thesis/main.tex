...
Chapter \ref{ch:online_fa} provides two algorithms which can be used to approximate the posterior distribution of the parameters of a neural network with a FA model, assuming that samples from the true posterior can be obtained via some separate procedure. This chapter describes an alternative method based on VI which learns a FA posterior directly.  

\section{Re-parameterisation Trick for Factor Analysis}

In Section \ref{sec:vi}, a general VI algorithm was outlined for learning an approximate posterior with a Gaussian form. Suppose that the approximate posterior is actually a FA model. That is, $q(\theta) = \mathcal{N}\big(\matr{c}, \matr{FF}^{\intercal} + \Psi\big)$ for some $\matr{c} \in \R^D$, $\matr{F} \in \R^{D \times K}$ and $D \times D$ diagonal matrix $\Psi$. Then the Monte Carlo gradient estimates in Equation (\ref{eqn:vi_derivatives}) are constructed from samples of the form 
\begin{equation}\label{eqn:vi_fa_derivatives}
	\nabla_{\matr{c}, \matr{F}, \Psi} \log q(\theta) 
	- \nabla_{\matr{c}, \matr{F}, \Psi} \log p(\theta) 
	-  \nabla_{\matr{c}, \matr{F}, \Psi} \log p(y_m | \matr{x}_m, \theta),
\end{equation}
where $\theta \sim \mathcal{N}\big(\matr{c}, \matr{FF}^{\intercal} + \Psi\big)$. Using the re-parameterisation trick with Equation (\ref{eqn:fa_model}), the sampling operation can be made an explicit function of $\matr{c}$, $\matr{F}$ and $\Psi$ by setting
\begin{equation}\label{eqn:fa_reparam_trick}
	\theta = \matr{Fh} + \matr{c} + \Psi^{1/2} \matr{z}
\end{equation}
from some $\matr{h} \sim \mathcal{N}(\matr{0}^K, \matr{I}^{K \times K})$ and $\matr{z} \sim \mathcal{N}(\matr{0}^D, \matr{I}^{D \times D})$. Given $\theta$, the partial derivatives in Equation (\ref{eqn:vi_fa_derivatives}) can then be evaluated. 

\section{Computing the Partial Derivatives}

Having generated $\theta$ according to Equation (\ref{eqn:fa_reparam_trick}),
\begin{align}
\begin{split}
	\log q(\theta)
...
\end{split}
\end{align}
Since these derivatives do not depend on $\matr{h}$ and $\matr{z}$, and using the fact that $\nabla_\matr{x} \E[f(\matr{x})] = \E[\nabla_\matr{x} f(\matr{x})]$, it follows that
\begin{equation}
	\nabla_{\matr{c}} \E[\log q(\theta)] = \matr{0}^D
\end{equation}
\begin{equation}
	\nabla_{\matr{F}} \E[\log q(\theta)] = \matr{0}^{D \times K}
\end{equation}
\begin{equation}
	\nabla_{\Psi} \E[\log q(\theta)] = -\frac{1}{2} \Psi^{-1}.
\end{equation}
This means that the partial derivatives of $\log q(\theta)$ can be computed exactly without the need to form a Monte Carlo estimate from samples of $\theta$. 

Now let the prior be $p(\theta) = \mathcal{N}(\matr{0}, \alpha^{-1} \matr{I})$ for some precision $\alpha > 0$. Then
\begin{align}
\begin{split}
	\log p(\theta)
...
\end{equation}
Since $\matr{h}$ and $\matr{z}$ are independent variables with zero mean and unit covariance, it follows that
\begin{equation}
	\nabla_\matr{c} \E[\log p(\theta)] = \alpha (\matr{Fh} + \matr{c}),
\end{equation}
\begin{equation}
	\nabla_\matr{F} \E[\log p(\theta)] = \alpha \matr{F},
\end{equation}
\begin{equation}
	\nabla_\Psi \E[\log p(\theta)] = \Psi^{1/2}.
\end{equation}


\section{Partial Derivatives of the Likelihood}
