...
An alternative strategy is to approximate the posterior directly with a Gaussian distribution $q(\theta) = \mathcal{N}(\mu, \Sigma)$, where the parameters $\mu$ and $\Sigma$ are chosen to minimise some measure of distance between $q(\theta)$ and $p(\theta | \mathcal{D})$. Because $q(\theta)$ is essentially a variable in an optimisation procedure, this approach is known as \emph{variational inference} (VI) \cite{barber2007}. A common measure of distance between two distributions is the Kullback-Leibler (KL) divergence \cite{barber2007}, which in this case is
\begin{align}
\begin{split}
...
\subsection{Re-parameterisation Trick for Factor Analysis}

In Section \ref{sec:vi}, a general VI algorithm was outlined for learning an approximate Gaussian posterior of the parameter vector of a neural network. The variational auto-encoder is based on this approach, but with a diagonal covariance matrix \cite{kingma2013}. Suppose instead that the approximate posterior is a FA model. That is, $q(\theta) = \mathcal{N}\big(\matr{c}, \matr{FF}^{\intercal} + \Psi\big)$ for some $\matr{c} \in \R^D$, $\matr{F} \in \R^{D \times K}$ and $D \times D$ diagonal matrix $\Psi$. Then the gradient of the VI objective in Equation (\ref{eqn:vi_objective}) becomes 
\begin{equation}\label{eqn:vi_fa_derivatives}
	...
	-  \nabla_{\matr{c}, \matr{F}, \Psi} \E_{q(\theta)} [\log p(y_m | \matr{x}_m, \theta)],
\end{equation}
where the expectation is taken over samples $\theta \sim \mathcal{N}\big(\matr{c}, \matr{FF}^{\intercal} + \Psi\big)$. Using the re-parameterisation trick to re-write Equation (\ref{eqn:fa_model}), the sampling operation can be made an explicit function of $\matr{c}$, $\matr{F}$ and $\Psi$ by setting
\begin{equation}\label{eqn:fa_reparam_trick}
	\theta = \matr{Fh} + \matr{c} + \Psi^{1/2} \matr{z}
\end{equation}
from some $\matr{h} \sim \mathcal{N}(\matr{0}^K, \matr{I}^{K \times K})$ and $\matr{z} \sim \mathcal{N}(\matr{0}^D, \matr{I}^{D \times D})$. Using this trick, the partial derivatives in Equation (\ref{eqn:vi_fa_derivatives}) can either be evaluated exactly or approximated by Monte Carlo estimates. 

\subsection{Partial Derivatives of the Variational Distribution}

Let $\theta$  be generated according to Equation (\ref{eqn:fa_reparam_trick}). Then
\begin{align}
\begin{split}
	\log q(\theta)
	& = -\frac{1}{2} (\theta - \matr{Fh} - \matr{c})^\intercal \Psi^{-1} (\theta - \matr{Fh} - \matr{c}) - \frac{1}{2} \log |\Psi| - \frac{D}{2} \log 2\pi \\
	& = -\frac{1}{2} (\Psi^{1/2} \matr{z})^\intercal \Psi^{-1} (\Psi^{1/2} \matr{z}) - \frac{1}{2} \log |\Psi| - \frac{D}{2} \log 2\pi \\
	& = -\frac{1}{2} \matr{z}^\intercal (\Psi^{1/2} \Psi^{-1} \Psi^{1/2}) \matr{z} - \frac{1}{2} \log |\Psi| - \frac{D}{2} \log 2\pi \\
	& = -\frac{1}{2} \matr{z}^\intercal \matr{z} - \frac{1}{2} \log |\Psi| - \frac{D}{2} \log 2\pi.
\end{split}
\end{align}
Hence, $\nabla_{\matr{c}} \log q(\theta) = \matr{0}^D$, $\nabla_{\matr{F}} \log q(\theta) = \matr{0}^{D \times K}$ and 
\begin{align}
\begin{split}
	\nabla_{\Psi} \log q(\theta)
	& = \nabla_\Psi \Big( -\frac{1}{2} \log |\Psi| \Big) \\
	& = -\frac{1}{2} \Psi^{-1},
\end{split}
\end{align}
which follows from Equation (\ref{eqn:derivatives_wrt_Psi_2}). Since these derivatives do not depend on $\matr{h}$ and $\matr{z}$, and using the fact that $\nabla_\matr{x} \E[f(\matr{x})] = \E[\nabla_\matr{x} f(\matr{x})]$ for any differentiable function $f$, 
\begin{equation}
	\nabla_{\matr{c}} \E_{q(\theta)} [\log q(\theta)] = \matr{0}^D,
\end{equation}
\begin{equation}
	\nabla_{\matr{F}} \E_{q(\theta)} [\log q(\theta)] = \matr{0}^{D \times K},
\end{equation}
\begin{equation}
	\nabla_{\Psi} \E_{q(\theta)} [\log q(\theta)] = -\frac{1}{2} \Psi^{-1}.
\end{equation}
...
\begin{split}
	\log p(\theta)
	& = -\frac{1}{2} (\matr{Fh} + \matr{c} + \Psi^{1/2} \matr{z})^\intercal \alpha \matr{I} (\matr{Fh} + \matr{c} + \Psi^{1/2} \matr{z})  - \frac{1}{2} \log |\alpha^{-1} \matr{I}| - \frac{D}{2} \log 2\pi \\
	& = -\frac{\alpha}{2} (\matr{Fh} + \matr{c} + \Psi^{1/2} \matr{z})^\intercal (\matr{Fh} + \matr{c} + \Psi^{1/2} \matr{z}) + \frac{n}{2} \log \alpha - \frac{D}{2} \log 2\pi.
\end{split}
\end{align}
Hence, using the matrix calculus identities in \cite{petersen2012},
\begin{equation}
	\nabla_\matr{c} \log p(\theta) = -\alpha (\matr{Fh} + \matr{c} + \Psi^{1/2} \matr{z}),
\end{equation}
\begin{equation}
	\nabla_\matr{F} \log p(\theta) = -\alpha (\matr{Fh} + \matr{c} + \Psi^{1/2} \matr{z}) \matr{h}^\intercal,
\end{equation}
\begin{equation}
	\nabla_\Psi \log p(\theta) = -\alpha (\matr{Fh} + \matr{c} + \Psi^{1/2} \matr{z}) \matr{z}^\intercal.
\end{equation}
Since $\matr{h}$ and $\matr{z}$ are independent random variables with zero mean and unit covariance, it follows that
\begin{equation}
	\nabla_\matr{c} \E_{q(\theta)} [\log p(\theta)] = -\alpha (\matr{Fh} + \matr{c}),
\end{equation}
\begin{equation}
	\nabla_\matr{F} \E_{q(\theta)} [\log p(\theta)] = -\alpha \matr{F},
\end{equation}
\begin{equation}
	\nabla_\Psi \E_{q(\theta)} [\log p(\theta)] = -\alpha \Psi^{1/2}.
\end{equation}


\subsection{Partial Derivatives of the Likelihood}

Let the log-likelihood, $\log p(y_m | \matr{x}_m, \theta)$, be parameterised by a neural network with parameters $\theta$. Then the back-propagation algorithm \cite{rumelhart1986} can be used to compute $\nabla_\theta \log p(y_m | \matr{x}_m, \theta)$. Since $\theta = \matr{Fh} + \matr{c} + \Psi^{1/2} \matr{z}$, it follows from the chain rule of calculus that
\begin{equation}
	\nabla_{\matr{c}} \log p(y_m | \matr{x}_m, \theta) = \nabla_\theta \log p(y_m | \matr{x}_m, \theta),
\end{equation}
\begin{equation}
	\nabla_{\matr{F}} \log p(y_m | \matr{x}_m, \theta) = \nabla_\theta \log p(y_m | \matr{x}_m, \theta) \matr{h}^\intercal,
\end{equation}
\begin{equation}
	\nabla_{\Psi} \log p(y_m | \matr{x}_m, \theta) = \frac{1}{2} \nabla_\theta \log p(y_m | \matr{x}_m, \theta) \matr{z}^\intercal \Psi^{-1/2}.
\end{equation}
...
As in Algorithm \ref{alg:gradient_fa}, it is far more efficient to work with $\psi = \diag(\Psi)$ rather than $\Psi$ itself. The required partial derivatives with respect to $\psi$ are
\begin{equation}
	\nabla_{\psi} \E_{q(\theta)} [\log q(\theta)] = -\frac{1}{2} \psi^{-1},
\end{equation}
\begin{equation}
...
\end{equation}
\begin{equation}
	\nabla_{\psi} \log p(y_m | \matr{x}_m, \theta) = \frac{1}{2} \nabla_\theta \log p(y_m | \matr{x}_m, \theta) \odot \big(\matr{z} \odot \psi^{-1/2}\big).
\end{equation}
Also, the re-parameterisation $\psi = \exp \beta$ can again be used to ensure that the variances remain positive. Similar to Equation (\ref{eqn:efficient_derivatives_wrt_beta}), it then follows from the chain rule that the corresponding partial derivatives with respect to $\beta$ are 
\begin{equation}
	\nabla_{\beta} \E_{q(\theta)} [\log q(\theta)] = -\frac{1}{2},
\end{equation}
\begin{equation}
	\nabla_\beta \E_{q(\theta)} [\log p(\theta)] = -\alpha \psi^{3/2},
\end{equation}
\begin{equation}
	\nabla_{\beta} \log p(y_m | \matr{x}_m, \theta) = \frac{1}{2} \nabla_\theta \log p(y_m | \matr{x}_m, \theta) \odot \big(\matr{z} \odot \psi^{1/2}\big).
\end{equation}
Pseudo code for the practical implementation is given in Algorithm \ref{alg:aevb_fa}.

\begin{algorithm}[!htbp] 
	\caption{Auto-Encoding Variational Bayes with Factor Analysis Posterior}
	\label{alg:aevb_fa}
	\begin{algorithmic}[1]
		\Require{Dataset $\mathcal{D} = \{(\matr{x}_n, y_n)\}_{n=1}^N$, observation dimension $D$, latent dimension $K$, learning rate $\eta$} 
		...
		\For {$t=1,\dots,T$}
			...
			\State $\matr{g}_\theta \leftarrow \frac{1}{|\mathcal{B}|} \sum_{(\matr{x}, y) \in \mathcal{B}} \nabla_\theta \log p(y | \matr{x}, \theta)$ (via back-propagation)
			\State $\matr{g}_\matr{c} \leftarrow \alpha (\matr{Fh} + \matr{c}) - \matr{g}_\theta$
			\State $\matr{G}_\matr{F} \leftarrow \alpha \matr{F} - \matr{g}_\theta \matr{h}^\intercal$
			\State $\matr{g}_\beta \leftarrow -\frac{1}{2} + \alpha \psi^{3/2} - \frac{1}{2} \nabla_\theta \log p(y_m | \matr{x}_m, \theta) \odot \big(\matr{z} \odot \psi^{1/2}\big)$
			\State $\matr{c} \leftarrow \matr{c} - \eta \matr{g}_\matr{c}$
			\State $\matr{F} \leftarrow \matr{F} - \eta \matr{G}_\matr{F}$
			\State $\beta \leftarrow \beta - \eta \matr{g}_\beta$
			\State $\psi \leftarrow \exp \beta$
		\EndFor
...
\section{Sampling from the Posterior}\label{sec:linear_regression_posterior_experiments}
...
\end{equation}
for some $\lambda > 0$. Setting $\lambda = \frac{\alpha}{\beta}$, the optimal $\theta$ found by L2-regularised linear regression is the same as the MAP estimate of $\theta$ in Bayesian linear regression with prior $p(\theta) = \mathcal{N}\big(\matr{0}, \alpha^{-1} \matr{I} \big)$. This means that, given data $\mathcal{D}$ and a particular value of $\alpha$, the SGD iterates sampled while minimising Equation (\ref{eqn:regularised_linear_regression}) with $\lambda = \frac{\alpha}{\beta}$ should in theory by representative of the true posterior $p(\theta | \mathcal{D})$. The purpose of the experiments in this section is to test this hypothesis. 
...
These results suggest that, in general, SGD does not sample parameter vectors from the true posterior distribution. This is a very simple example of a linear model in two dimensions. Even when tuning the learning rate and regularisation strength, it is not possible to obtain samples which are representative of the true posterior. For the certain values of $\lambda$ (but not the theoretically motivated one), the mean solution $\theta_{\text{SWA}}$ is a good approximation of the true posterior mean, but the same cannot be said for the empirical covariance. The most that can be expected from the online FA algorithms is that they fit the sampled parameter vectors well. Therefore, given the data observed in Figure \ref{fig:linear_regression_pdfs}, it would be unreasonable to expect them to learn the true posterior distribution. In the case of a high-dimensional multi-layer neural network, approximating the true posterior would be even more unlikely, since in that case the posterior cannot be compute in closed form, which makes hyperparameter tuning far more difficult. These results suggest that, in general, SWAG does not actually do approximate Bayesian inference, contrary to what is claimed in \cite{maddox2019}.

It was also observed in \cite{mandt2017} that the empirical covariance of the linear regression SGD iterates does not match the true posterior covariance. As such, the authors proposed a different technique for posterior sampling called \emph{finite window iterate averaging}. Instead of using the individual parameter vectors sampled after each mini-batch update of SGD, this algorithm averages the parameter vectors in fixed-length disjoint windows. These average parameter vector are then considered samples from the posterior. Figure \ref{fig:linear_regression_average_iterates} shows an example of such samples for the same learning rate and regularisation strength as that in Figure \ref{fig:linear_regression_pdfs} (b), but with a batch size of 10 and where each red dot is the mean of 50 consecutive SGD iterates. These samples are a much better representation of the true posterior. However, to obtain such a result required significant tuning of both the batch size and window size. In the case of non-linear neural networks for which the true posterior is not known, this would not be possible. Some theoretical results for the ``optimal" relationship between the sample size, batch size, window size and learning rate are given in \cite{mandt2017}. However, these require knowledge of the minimum and maximum eigenvalues of the true posterior covariance, which is in general not available. Therefore, this algorithm does not seem to be a viable option in practice. 

\begin{figure}[!htbp] 
	\begin{center}
		\includegraphics[width=90mm]{plots/linear_model_average_weight_iterates__lr=0.1__lambda=0.001.png} 
	\end{center}
	\caption{Samples constructed via finite window iterate averaging. Each red dot is the mean of the linear regression parameter vectors sampled during 50 consecutive mini-batch updates of SGD, with a batch size of 10, a learning rate of 0.1 and the regularisation strength set to 0.001. The contours show the pdf of the true posterior distribution. The white dot denotes the pre-trained parameter vector and the black star is the empirical mean of the red dots.}
