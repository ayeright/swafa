...
\begin{figure}[!htbp] 
	...
	\end{tabular}
	\caption{Parameter vectors sampled while training a linear regression model via SGD, for different combinations of learning rate and regularisation strength. These are shown as red dots, plotted on top of the contours of the true posterior distribution pdf. In each plot, the white dot denotes the pre-trained parameter vector and the black star is the empirical mean of the red dots.}
	\label{fig:linear_regression_pdfs}
\end{figure}

These results suggest that SGD does not sample parameter vectors from the true posterior distribution. This is a very simple example of a linear model in two dimensions. Even when tuning the learning rate and regularisation strength, it is not possible to obtain samples which are representative of the true posterior. For the right value of $\lambda$, the mean solution $\theta_{\text{SWA}}$ is a good approximation of the true posterior mean, but the same cannot be said for the empirical covariance. The most that can be expected from the online FA algorithms is that they fit the sampled parameter vectors well. Therefore, given the data observed in Figure \ref{fig:linear_regression_pdfs}, it would be unreasonable to expect them to learn the true posterior distribution. In the case of a high-dimensional multi-layer neural network, approximating the true posterior would be even more unlikely, since in that case the posterior cannot be compute in closed form, which makes hyperparameter tuning far more difficult. These results suggest that SWAG does not actually do approximate Bayesian inference, contrary to what is claimed in \cite{maddox2019}.

It was also observed in \cite{mandt2017} that the empirical covariance of the SGD iterates does not match the true posterior covariance. The authors proposed a different technique for posterior sampling called \emph{finite window iterate averaging}. Instead of using the individual parameter vectors sampled after each mini-batch update of SGD, this algorithm averages the parameter vectors in fixed-length disjoint windows. These average parameter vector are then considered samples from the posterior. Figure \ref{fig:linear_regression_average_iterates} shows an example of such samples for the same learning rate and regularisation strength as that in Figure \ref{fig:linear_regression_pdfs} (b), but with a batch size of 10 and where each red dot is the mean of 50 consecutive SGD iterates. These samples are a much better representation of the true posterior. However, to obtain such a result required significant tuning of both the batch size and window size. In the case of non-linear neural networks for which we do not know the true posterior, this would not be possible. Some theoretical results for the relationship between the sample size, batch size, window size and learning rate are given in \cite{mandt2017}. However, these require knowledge of the minimum and maximum eigenvalues of the true posterior covariance, which is in general not available. Therefore, this algorithm does not seem to be a viable option in practice. 
...
\begin{figure}[!htbp] 
	...
	\end{tabular}
	\caption{Average test mean squared error of linear regression models on four UCI regression datasets. Results are show for four different prediction methods: using the pre-trained parameter vector, using the SWA solution and two ensembles formed by sampling parameter vectors from FA models fit to the SGD iterates, one via online SGA and another learned via online EM. The markers show the average test error over 10 folds of cross-validation, and standard error bars are also plotted.}
	\label{fig:linear_regression_mse}
\end{figure}

These results suggest that, in the case of linear regression, fitting a FA model to the SGD iterates via online EM can lead to more accurate predictions compared to standard SGD. However, this approach does not seem to improve on the much simpler SWA solution. Note that, in the case of linear regression, an ensemble of $M$ parameter vectors sampled from a FA model can be written as 
\begin{equation}
...
\end{equation}
Hence, if the learned FA model is a good approximation of the empirical Gaussian distribution of the SGD iterates, $\theta_{\text{FA}}$ will be similar to $\theta_{\text{SWA}}$ for large $M$. This observation suggests that, if the accuracy of linear regression predictions is all that matters, the best approach is to use $\theta_{\text{SWA}}$.
...
% 			Dataset & Pre-trained & SWA & SGA-FA & EM-FA \\ [0.5ex] 
% 			\hline\hline
%			Energy Efficiency & $10.13 \pm 0.55$ & $8.74 \pm 0.54$ & $8.93 \pm 0.53$ & $8.73 \pm 0.54$ \\
%			Boston Housing & $ \pm $ & $ \pm $ & $ \pm $ & $ \pm $ \\
%			Concrete Strength & $ \pm $ & $ \pm $ & $ \pm $ & $ \pm $ \\
%			Yacht Hydro. & $ \pm $ & $ \pm $ & $ \pm $ & $ \pm $ \\ [1ex] 
%			\hline
%		\end{tabular}
%		\caption{}
%		\label{table:linear_regression_mse}
