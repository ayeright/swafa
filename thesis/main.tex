...
\end{equation}
Note that, in practice, computing Equation (\ref{eqn:variational_params}) would be unfeasible for large $d$ as it involves multiplying by the $d\times d$ matrix $\Psi^{-1}$. However, using the fact that $\Psi^{-1}$ is a diagonal matrix,
\begin{equation}
...
\begin{algorithm}[!htbp] 
	...
	\begin{algorithmic}[1]
		...
		\EndFor
		\State $\theta_{\text{SWA}} = \overline{\theta}_t$
