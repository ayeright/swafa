...
\end{equation}
from some $\matr{h} \sim \mathcal{N}(\matr{0}^K, \matr{I}^{K \times K})$ and $\matr{z} \sim \mathcal{N}(\matr{0}^D, \matr{I}^{D \times D})$, where the exponent is applied element-wise. Using this trick, the partial derivatives in Equation (\ref{eqn:vi_fa_derivatives}) can either be evaluated exactly or approximated by Monte Carlo estimates. 

\subsection{Partial Derivatives of the Variational Distribution}

Let $\theta$  be generated according to Equation (\ref{eqn:fa_reparam_trick}). Then from Equation (\ref{eqn:fa_cond_dist}),
\begin{align}
\begin{split}
	\log q(\theta)
...
\begin{split}
	...
	& = -\frac{\alpha}{2} (\matr{Fh} + \matr{c} + \Psi^{1/2} \matr{z})^\intercal (\matr{Fh} + \matr{c} + \Psi^{1/2} \matr{z}) + \frac{D}{2} \log \alpha - \frac{D}{2} \log 2\pi.
\end{split}
\end{align}
Hence, using the matrix calculus identities from \cite{petersen2012},
\begin{equation}
...
\end{equation}
\begin{equation}
	\nabla_\Psi \log p(\theta) = \diag\Big(\diag\Big(-\frac{\alpha}{2} (\matr{Fh} + \matr{c} + \Psi^{1/2} \matr{z}) \matr{z}^\intercal \Psi^{-1/2}\Big)\Big).
\end{equation}
...
Let the log-likelihood, $\log p(\mathcal{D} | \theta)$, be parameterised by a neural network with weights $\theta$. Then the back-propagation algorithm \cite{rumelhart1986} can be used to compute $\nabla_\theta \log p(\mathcal{D} | \theta)$. Since $\theta = \matr{Fh} + \matr{c} + \Psi^{1/2} \matr{z}$, it follows from the chain rule of calculus that
\begin{equation}
...
\begin{algorithm}[!htbp] 
	...
	\label{alg:aevb_fa}
	\begin{algorithmic}[1]
		\Require{Dataset $\mathcal{D} = \{(\matr{x}_n, y_n)\}_{n=1}^N$, observation dimension $D$, latent dimension $K$, precision $\alpha > 0$, mini-batch size $M$, learning rate $\eta > 0$} 
		...
		\For {$t=1,\dots,T$}
			...
			\State $\matr{g}_\beta \leftarrow -\frac{1}{2} + \frac{\alpha}{2} \psi - \frac{1}{2} \matr{g}_\theta \odot \big(\psi^{1/2} \odot \matr{z}\big)$
