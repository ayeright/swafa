...
\begin{equation}
	...
	\text{sum}\big(\matr{F} \odot \matr{F}, \text{ dim} = 1\big)
	 + \psi
	 - \text{sum}\big(\matr{F} \odot (\matr{F} \matr{C}^\intercal \Sigma), \text{ dim} = 1\big),
\end{equation}
...
\begin{equation}
	\nabla_\beta \E_{q(\theta)} [\log q(\theta)] 
	 = \frac{1}{2} \psi^{-1} \odot \matr{s} - \frac{1}{2}.
\end{equation}
\begin{equation}
...
\begin{algorithm}[!htbp] 
	...
	\begin{algorithmic}[1]
		...
		\For {$t=1,\dots,T$}
			...
			\State $\matr{A} \leftarrow \psi^{-1} \odot \matr{F}$
			\State $\matr{B} \leftarrow \matr{A}^\intercal \matr{F}$
			\State $\matr{C} = 2\matr{B} + \matr{I} - (\matr{B} + \matr{I}) \matr{B} \Sigma$
			\State $\matr{s} \leftarrow \text{sum}\big(\matr{F} \odot \matr{F}, \text{ dim} = 1\big) + \psi - \text{sum}\big(\matr{F} \odot (\matr{F} \matr{C}^\intercal \Sigma), \text{ dim} = 1\big)$
			...
			\State $\matr{g}_\beta \leftarrow \frac{1}{2}\psi^{-1} \odot \matr{s} -\frac{1}{2} + \frac{\alpha}{2} \psi + \frac{1}{2} \matr{g}_\theta \odot \big(\psi^{1/2} \odot \matr{z}\big)$
