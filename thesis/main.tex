...
\end{split}
\end{align} 
From the E-step of the EM algorithm in \cite{barber2007}, $p(\matr{h}_t | \theta_t, \matr{F}, \Psi) \propto \mathcal{N}(\matr{m}_t, \Sigma)$, where
...
\begin{equation}\label{eqn:derivatives_wrt_F}
	\nabla_{\matr{F}} \log p(\theta_t | \matr{F}, \Psi) 
...
\begin{equation}
	\matr{F}^\intercal \Psi^{-1} = \matr{F}^\intercal \odot \diag(\Psi^{-1})^\intercal,
\end{equation}
where $\odot$ denotes the element-wise matrix product and the vector $\diag(\Psi^{-1})^\intercal$ is \emph{broadcast} with the rows of the matrix $\matr{F}^\intercal$. When applied to a $d$-length vector, $\diag(\cdot)$ represents the $d \times d$ diagonal matrix with the vector on its diagonal, and when applied to a $d \times d$ matrix, $\diag(\cdot)$ represents the $d$-length vector consisting of the diagonal entries of the matrix. Now, setting $\matr{A} = \matr{F}^\intercal \odot \diag(\Psi^{-1})^\intercal$, it follows that 
\begin{equation}\label{eqn:efficient_sigma}
	\Sigma = (\matr{I} + \matr{A} \matr{F})^{-1},
\end{equation}
and then
\begin{equation}\label{eqn:efficient_m}
	\matr{m}_t = \Sigma \matr{A} \matr{d}_t.
\end{equation}
...
\begin{equation}\label{eqn:derivatives_wrt_Psi_1}
	...
	= -\diag\Big(\diag(\Psi^{-2}) \odot \big((\matr{d}_t - \matr{Fh}_t) \odot (\matr{d}_t - \matr{Fh}_t)\big)\Big).
\end{equation}
...
\begin{equation}
	...
	= \frac{1}{2} \diag\Big(\diag(\Psi^{-2}) \odot \big((\matr{d}_t - \matr{Fh}) \odot (\matr{d}_t - \matr{Fh})\big)\Big) - \frac{1}{2}\Psi^{-1}.
\end{equation}
...
\begin{split}
	...
	& = \E \Big[ \diag\Big(\diag(\Psi^{-2}) \odot \big((\matr{d}_t - \matr{Fh}_t) \odot (\matr{d}_t - \matr{Fh}_t)\big)\Big) - \Psi^{-1} \Big] \\
	& = \diag\Big(\E \big[\diag(\Psi^{-2}) \odot \big((\matr{d}_t - \matr{Fh}_t) \odot (\matr{d}_t - \matr{Fh}_t)\big]\Big) - \E \big[ \Psi^{-1} \big] \\
	& = \diag\Big(\diag(\Psi^{-2}) \odot \E \big[\big((\matr{d}_t - \matr{Fh}_t) \odot (\matr{d}_t - \matr{Fh}_t)\big]\Big) - \Psi^{-1}.
\end{split}
\end{align} 
...
\begin{split}
	\E \big[\big((\matr{d}_t - \matr{Fh}) \odot (\matr{d}_t - \matr{Fh}_t)\big] 
	& = \E \big[\matr{d}_t \odot \matr{d}_t \big] - 2\E \big[ \matr{d}_t \odot \matr{Fh}_t \big] + \E \big[ \matr{Fh}_t \odot \matr{Fh}_t \big] \\
	& = \matr{d}_t \odot \matr{d}_t - 2\matr{d}_t \odot \matr{F} \E \big[ \matr{h}_t \big] + \E \big[ \diag(\matr{F}\matr{h}_t \matr{h}_t^\intercal  \matr{F}^\intercal) \big] \\
...
\begin{split}\label{eqn:derivatives_wrt_Psi}
	\nabla_{\Psi} \log p(\theta_t | \matr{F}, \Psi) 
	& = \frac{1}{2} \diag\Big(\diag(\Psi^{-2}) \odot \big(\matr{d}_t \odot \matr{d}_t - 2\matr{d}_t \odot \matr{F} \matr{m}_t \\
...
\begin{algorithm}[!htbp] 
	...
	\begin{algorithmic}[1]
		...
		\State Initialise $\matr{F} \in \R^{d \times K}, \Psi = \diag(x_1,\dots,x_d), \overline{\theta}_0 = \matr{0}^d$
		\For {$t=1,\dots,T$}
			\State Sample observation $\theta_t \in \R^d$
			\State Update running average: 
			...
			\State $\Psi^{-1} \leftarrow \diag\Big(\frac{1}{x_1}, \dots, \frac{1}{x_d}\Big)$ 
			\State $\Psi^{-2} \leftarrow \diag\Big(\frac{1}{x_1^2}, \dots, \frac{1}{x_d^2}\Big)$
			\State $\matr{A} \leftarrow \matr{F}^\intercal \odot \diag(\Psi^{-1})^\intercal$
			\State $\Sigma \leftarrow (\matr{I} + \matr{A} \matr{F})^{-1}$ (Equation (\ref{eqn:efficient_sigma}))
			\State $\matr{m}_t \leftarrow \Sigma \matr{A} \matr{d}_t$ (Equation (\ref{eqn:efficient_m}))
...
\end{equation}
where $\matr{m}_t$ and $\Sigma$ are given in Equation (\ref{eqn:variational_params}). Note that this optimisation can be performed separately for each $\theta_t$ as it is sampled, using the estimates of $\matr{F}$ and $\Psi$ on iteration $t$. However, in batch EM all $q(\matr{h}_t | \theta_t)$ are update on every iteration. This is clearly not possible in an online algorithm which discards $\theta_t$ before sampling the next observation. Therefore, as a compromise, in the online version each $q(\matr{h}_t | \theta_t)$ will be computed once only, on iteration $t$, and held fixed thereafter. The only other detail is that the batch algorithm uses $\theta_{\text{SWA}}$ to compute $\matr{m}_t$. As $\theta_{\text{SWA}}$ is not available during training, it will be replaced by the running average of the neural network's parameter vectors, as in the online gradient-based FA algorithm from Section \ref{sec:gradient_fa}.
...
\begin{algorithm}[!htbp] 
	...
	\begin{algorithmic}[1]
		...
		\State Initialise $\overline{\theta}_0 = \matr{0}^d, \matr{A} = \matr{0}^{d \times K}, 
			\matr{B} = \matr{0}^{K \times K}, \matr{d} = \matr{0}^d$
		\For {$t=1,\dots,T$}
			\State Sample observation $\theta_t \in \R^d$
			\State Update running average: 
				$\overline{\theta}_t \leftarrow  \overline{\theta}_{t-1} + \frac{1}{t}\big(\overline{\theta}_t - \overline{\theta}_{t-1}\big)$
			...
			\State $\Sigma \leftarrow (\matr{I} + \matr{A} \matr{F})^{-1}$ (Equation (\ref{eqn:efficient_sigma}))
			\State $\matr{m}_t \leftarrow \Sigma \matr{A} \matr{d}_t$ (Equation (\ref{eqn:efficient_m}))
			\State $\matr{A} \leftarrow \matr{A} + \frac{1}{t} (\matr{d}_t \matr{m}_t^\intercal - \matr{A})$
			\State $\matr{B} \leftarrow \matr{B} + \frac{1}{t} (\matr{m}_t \matr{m}_t^\intercal - \matr{B})$
			\State $\matr{d} \leftarrow \matr{d} + \frac{1}{t} (\matr{d}_t^2 - \matr{d})$
			\State $\matr{H} \leftarrow \Sigma + \matr{B}$
			\State $\matr{F} \leftarrow \matr{A}\matr{H}^{-1}$
			\State $\Psi \leftarrow 
				\diag \big( \matr{d}
				+ \text{sum} (\matr{F} \matr{H} \odot \matr{F} -2\matr{F} \odot \matr{A}, \text{ dim} = 1) \big)$ 
		\EndFor
		\State $\theta_{\text{SWA}} \leftarrow \overline{\theta}_t$
		\State \Return $\theta_{\text{SWA}}, \matr{F}, \Psi$
	\end{algorithmic}
