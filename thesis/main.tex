...
\begin{algorithm}[!htbp] 
	...
	\label{alg:vi_fa}
	\begin{algorithmic}[1]
		\Require{Dataset $\mathcal{D} = \{(\matr{x}_n, y_n)\}_{n=1}^N$, observation dimension $D$, latent dimension $K$, precision $\alpha > 0$, mini-batch size $M$, Monte Carlo average size $L$, learning rates $\eta_\matr{c},  \eta_\matr{F}, \eta_\beta > 0$} 
		...
		\For {$t = 1,\dots, T$}
			...
			\State $\hat{\matr{g}}_\theta \leftarrow -\frac{N}{M} \sum_{(\matr{x}, y) \in \mathcal{B}} \nabla_\theta \log p(y | \matr{x}, \theta)$ (via back-propagation)
			...
			\If {$t \bmod L = 0$}
				...
				\State $\psi \leftarrow \exp \beta$
				\State $\hat{\matr{g}}_\matr{c} \leftarrow \matr{0}^D$
				\State $\hat{\matr{G}}_\matr{F} \leftarrow \matr{0}^{D \times K}$
				\State $\hat{\matr{g}}_\beta \leftarrow \matr{0}^D$
				\State Update $\matr{A}, \matr{B}, \matr{C}, \matr{D}, \matr{s}$ according to lines 4-8
			\EndIf
		\EndFor
...
\begin{figure}[!htbp] 
	\begin{tabular}{cc}
		 ...
		 (a) Ground truth
		 & (b) VI-FA, latent dim = 2 \\
		 \includegraphics[width=70mm]{plots/linear_model_vi_posterior__alpha=0.01__beta=0.1__latent_dim=1.png} \\
		 (c) VI-FA, latent dim = 1 \\[6pt]
	\end{tabular}
	\caption{The true posterior pdf of a linear regression model with two learnable parameters, plus the pdfs of two FA models fit to the same data via VI. Both approximations are very similar to the ground truth, including the FA model with only a single latent dimension.}
