...
Using Equation (\ref{eqn:derivatives_wrt_F}) and the fact that $\nabla_\matr{x} \E[f(\matr{x})] = \E[\nabla_\matr{x} f(\matr{x})]$ for any differentiable function $f$,  it follows that
\begin{align}
...
\end{split}
\end{align}
Hence, substituting into Equation (\ref{eqn:vi_expected_derivatives_wrt_F}),
\begin{align}
\begin{split}
	\nabla_\matr{F} \E_{q(\theta)} [\log q(\theta)] 
	& = \Psi^{-1}\big( 
...
\end{split}
\end{align}
Now let $\matr{A} = \Psi^{-1} \matr{F} \in \R^{D \times K}$ and note that $\matr{A}^\intercal = \matr{F}^\intercal \Psi^{-1} \in \R^{K \times D}$. Setting $\matr{B} = \matr{A}^\intercal \matr{F} \in \R^{K \times K}$, 
\begin{align}
\begin{split}
	\nabla_\matr{F} \E_{q(\theta)} [\log q(\theta)] 
...
\end{split}
\end{align}
%Since $\matr{A} = \diag(\Psi^{-1}) \odot \matr{F}$, this expression can be computed without working with any $D \times D$ matrices. 
...
\end{split}
\end{align}
Letting $\matr{x} =  \E_{q(\theta)}[\matr{d} \odot \matr{d}] -2\E_{q(\theta)}[\matr{d} \odot (\matr{F} \matr{m})] + \diag(\matr{F} \Sigma \matr{F}^\intercal + \matr{F}  \E_{q(\theta)}[\matr{m} \matr{m}^\intercal] \matr{F}^\intercal)$,
\begin{align}
\begin{split}
	\matr{x} 
	 ...
	 & =\diag\big(\matr{FF}^{\intercal} + \Psi - \matr{F} \Sigma \matr{C} \matr{F}^\intercal \big),
\end{split}
\end{align}
where $\matr{C} = 2\matr{B} + \matr{I} - (\matr{B} + \matr{I}) \matr{B} \Sigma \in \R^{K \times K}$. Hence, substituting into Equation (\ref{eqn:vi_expected_derivatives_wrt_Psi}),
\begin{align}
\begin{split}
	\nabla_\Psi \E_{q(\theta)} [\log q(\theta)] 
	 & = \diag\Bigg(
...
%\begin{split}
%	\diag\big(\matr{FF}^{\intercal} + \Psi - \matr{F} \Sigma \matr{C} \matr{F}^\intercal \big)
%	& = \text{sum}\big(\matr{F} \odot \matr{F}, \text{ dim} = 1\big)
%	+ \diag{\Psi} \\
%	& \quad - \text{sum}\big(\matr{F} \odot (\matr{F} \matr{C}^\intercal \Sigma), \text{ dim} = 1\big)
%\end{split}
%\end{align}	
...
\end{equation}
from some $\matr{h} \sim \mathcal{N}(\matr{0}^K, \matr{I}^{K \times K})$ and $\matr{z} \sim \mathcal{N}(\matr{0}^D, \matr{I}^{D \times D})$, where the exponent is applied element-wise to $\Psi$. Given $\matr{h}$ and $\matr{z}$, this is a deterministic function of $\matr{c}$, $\matr{F}$ and $\Psi$. It then follows from the chain rule of calculus that
\begin{equation}
