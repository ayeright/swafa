...
\begin{equation}
	...
	\text{sum}\big(\matr{F} \odot \matr{F}, \text{ dim} = 1\big)
	 + \psi
	 - \text{sum}\big(\matr{F} \odot \matr{C}^\intercal, \text{ dim} = 1\big),
\end{equation}
...
\begin{algorithm}[!htbp] 
	...
	\begin{algorithmic}[1]
		...
		\For {$t=1,\dots,T$}
			...
			\State $\matr{s} \leftarrow \text{sum}\big(\matr{F} \odot \matr{F}, \text{ dim} = 1\big) + \psi - \text{sum}\big(\matr{F} \odot  \matr{C}^\intercal, \text{ dim} = 1\big)$
