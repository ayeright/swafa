...
Let $f(\matr{x}): \R^D \mapsto \R$ be a neural network with a single hidden layer. Neural network will be explained in more detail in Section \ref{sec:dnns}, but for now it's enough to know that the functional form of this model is
\begin{equation}
	f(\matr{x}) = b + \sum_{h=1}^H v_h \cdot g\Big(w_0 + \sum_{d=1}^D w_d x_d\Big),
\end{equation} 
where $H$ is the number of units in the hidden layer, the $w_d$, $v_h$ and $b$ are learnable parameters and $g(x):\R \mapsto \R$ is a non-linear \emph{activation} function \cite{goodfellow2016}.

Let $\theta \in \R^{(D + 1)h + h + 1}$ denote all the learnable parameters of the neural network. For two points $\matr{x}, \matr{x}' \in \R^D$, the covariance function of the neural network is defined as 
\begin{equation}
	\E_\theta[]
\end{equation} 
