...
Figure \ref{fig:linear_regression_pdfs} shows the pdfs of the true posterior and empirical distribution for each combination of the hyperparameters, for one of the ten trials in each experiment. Note that in each case the pre-trained parameter vector is very close to the true posterior mean. When $\lambda = 0.1$, SGD moves away from the pre-trained solution to a different region of the parameter space (subplots (a) and (c)). When $\lambda = 0.001$, SGD bounces around the pre-trained solution and the empirical mean of the parameter vectors, $\theta_{\text{SWA}}$, is right on the true posterior mean. However, when the learning rate is 0.1 the empirical covariance is in the wrong direction (subplot (b)), and when the learning rate is 0.01 the empirical covariance is far too narrow (subplot (d)).

%\renewcommand*\arraystretch{0.01}
\begin{figure}[!htbp] 
	\begin{tabular}{c}
		\includegraphics[width=130mm]{plots/linear_model_weight_iterates__lr=0.1__lambda=0.1.png} \\
		(a) Learning rate of 0.1 and $\lambda = 0.1$ \\
		\includegraphics[width=130mm]{plots/linear_model_weight_iterates__lr=0.1__lambda=0.001.png} \\
		(b) Learning rate of 0.1 and $\lambda = 0.001$ \\
		\includegraphics[width=130mm]{plots/linear_model_weight_iterates__lr=0.01__lambda=0.1.png} \\
		(c) Learning rate of 0.01 and $\lambda = 0.1$ \\
		\includegraphics[width=130mm]{plots/linear_model_weight_iterates__lr=0.01__lambda=0.001.png} \\
		(d) Learning rate of 0.01 and $\lambda = 0.001$ \\
	\end{tabular}
	\caption{Probability density functions for the true posterior distribution of the parameter vector of a linear regression model and the empirical distribution of the parameter vectors sampled during SGD, for different combinations of learning rate and regularisation strength. 
%	In each plot, the white dot denotes the pre-trained parameter vector, red dots are the sampled parameter vectors and the black star is the empirical mean of the red dots. Contours show the probability density of each distribution.
	}
