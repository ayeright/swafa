...
In Section \ref{sec:vi}, a general VI algorithm was outlined for learning an approximate posterior with a Gaussian form. Suppose that the approximate posterior is actually a FA model. That is, $q(\theta) = \mathcal{N}\big(\matr{c}, \matr{FF}^{\intercal} + \Psi\big)$. Then the Monte Carlo gradient estimates in Equation (\ref{eqn:vi_derivatives}) are constructed from samples of the form 
\begin{equation}
...
\begin{equation}
	\theta = \matr{Fh} + \matr{c} + \matr{z} \odot \sqrt{\diag({\Psi})}
\end{equation}
from some $\matr{h}, \matr{z} \sim \mathcal{N}(\matr{0}, \matr{I})$, where the square root is applied element-wise. 

\section{Partial Derivatives of Variational Distribution}

Having sampled $\theta$, the partial derivatives $\nabla_{\matr{c}, \matr{F}, \Psi} \log q(\theta)$ can be obtained by starting from modified versions of Equations (\ref{eqn:simplified_grad_log_likelihood}) and (\ref{eqn:log_fa_cond_dist}). Formally,  
\begin{equation}\label{eqn:partial_derivatives_of_q}
	\nabla_{\matr{c}, \matr{F}, \Psi} \log q(\theta)
	= \E_{p(\matr{h} | \theta, \matr{c}, \matr{F}, \Psi)} \big[ \nabla_{\matr{c}, \matr{F}, \Psi} \log p(\theta | \matr{h}, \matr{c}, \matr{F}, \Psi) \big],
\end{equation}
where 
\begin{equation}\label{eqn:log_fa_cond_dist_with_c}
	\log p(\theta | \matr{h}, \matr{c}, \matr{F}, \Psi)
	= -\frac{1}{2} (\theta - \matr{Fh} - \matr{c})^\intercal \Psi^{-1} (\theta - \matr{Fh} - \matr{c}) - \frac{1}{2} \log |\Psi| - \frac{D}{2} \log 2\pi.
\end{equation}
From here, the derivation of $\nabla_{\matr{F}, \Psi} \log q(\theta)$ is the same as that in Section \ref{sec:gradient_fa}, except all instances of $\theta_{\text{SWA}}$ are replaced by $\matr{c}$. To obtain $\nabla_{\matr{c}} \log q(\theta)$, first note that the partial derivates of Equation (\ref{eqn:log_fa_cond_dist_with_c}) with respect to $\matr{c}$ are 
\begin{equation}
	\nabla_{\matr{c}} \log p(\theta | \matr{h}, \matr{c}, \matr{F}, \Psi) = \Psi^{-1} (\theta - \matr{Fh} - \matr{c}),
\end{equation}
which follows from the fact that
\begin{equation}
	\nabla_\matr{s} (\matr{x} - \matr{s})^\intercal \matr{W} (\matr{x} - \matr{s}) = -2\matr{W} (\matr{x} - \matr{s})
\end{equation}
for any symmetric matrix $\matr{W}$ \cite{petersen2012}. Hence, substituting into Equation (\ref{eqn:partial_derivatives_of_q}),
\begin{align}
\begin{split}
	\nabla_{\matr{c}} \log q(\theta) 
	& = \E_{p(\matr{h} | \theta, \matr{c}, \matr{F}, \Psi)} \big[ \Psi^{-1} (\theta - \matr{Fh} - \matr{c}) \big] \\
	& = \Psi^{-1} (\theta - \matr{F} \E_{p(\matr{h} | \theta, \matr{c}, \matr{F}, \Psi)}[\matr{h}]  - \matr{c}) \\
	& = \Psi^{-1} (\theta - \matr{Fm} - \matr{c}),
\end{split}
\end{align}
where $\matr{m}$ is given by replacing $\theta_{\text{SWA}}$ with $\matr{c}$ in Equation \ref{eqn:variational_params}.
