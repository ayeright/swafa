...
\begin{figure}[!htbp] 
	...
	\end{tabular}
	\caption{Probability density functions for the true posterior distribution of the parameter vector of a linear model and the empirical distribution of the parameter vectors sampled during SGD, for different combinations of learning rate and regularisation strength. In each plot, the white dot denotes the pre-trained parameter vector, red dots are the sampled parameter vectors and the black star is the empirical mean of the red dots. Contours show the probability density of each distribution.}
	\label{fig:linear_regression_pdfs}
\end{figure}

These results suggest that SGD does not sample parameter vectors from the true posterior distribution. This is a very simple example of a linear model in two dimensions. Even when tuning the learning rate and regularisation strength, it is not possible to obtain samples which are representative of the true posterior. For the right value of $\lambda$, the mean solution $\theta_{\text{SWA}}$ is a good approximation of the true posterior mean, but the same cannot be said for the empirical covariance. The most that can be expected from the online FA algorithms is that they fit the sampled parameter vectors well. Therefore, given the data observed in Figure \ref{fig:linear_regression_pdfs}, it would be unreasonable to expect them to learn the true posterior distribution. In the case of a high-dimensional multi-layer neural network, approximating the true posterior would be even more unlikely, since in that case the posterior cannot be compute in closed form, which makes hyperparameter tuning far more difficult. These results suggest that SWAG and similar techniques do not actually do approximate Bayesian inference, but are rather elaborate ensemble methods. 
