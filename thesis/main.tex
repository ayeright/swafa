...
\end{equation}
The full derivation is given in Appendix \ref{app:online_fa_sga_derivatives}.
...
\end{equation}
Instead of re-computing these sums on each iteration, they can be updated incrementally. The derivation of the update rule for $\overline{\matr{A}}_t$ is 
\begin{align}
\begin{split}
	\overline{\matr{A}}_t 
...
\end{split}
\end{align}
The update rule for $\overline{\matr{H}}_t$ is similar. Then the estimates of $\matr{F}$ and $\Psi$ on iteration $t$ become $\matr{F} = \overline{\matr{A}}_t \overline{\matr{H}}_t^{-1}$ and
...
Further analysis revealed that this is indeed the case. Figure \ref{fig:linear_regression_pdfs} shows the SGD iterates plotted on top of the true posterior for different combinations of the learning rate and the regularisation strength. Note that in each case the pre-trained parameter vector is close to the true posterior mean. However, when $\lambda = 0.1$ (subplots (a) and (c)), SGD moves away from the pre-trained solution to a different region of the parameter space. When $\lambda = 0.001$ (subplots (b) and (d)), SGD does bounce around the true posterior mean, but the empirical covariance is in the wrong direction and far too narrow when the learning rate is $0.001$.
...
\begin{figure}[!htbp] 
	...
	%These are shown as red dots, plotted on top of the contours of the pdf of the true posterior distribution. In each plot, the white dot denotes the pre-trained parameter vector and the black star is the empirical mean of the red dots.
	}
	\label{fig:linear_regression_pdfs}
\end{figure}

The most that can be expected of online EM is that it fits the SGD iterates well. If the iterates are not representative of the true posterior, applying online EM is a futile endeavour. In these experiments it was not even possible to obtain representative samples of a very simple linear regression posterior with only two dimensions. Perhaps for some combination of the batch size, learning rate and regularisation strength this is possible. However, in the case of a non-linear neural network the true posterior is not known, making hyperparameter optimisation extremely difficult. Therefore, methods based on SWAG are unlikely to achieve good posterior covariance estimation in practice. Fortunately, there are more direct approaches which do not rely on a separate posterior sampling procedure. One such algorithm is described next in Chapter \ref{ch:vifa}.
...
\end{equation}
Note that $\matr{g}_{\theta_l}$ is the average negative log-likelihood of $\theta_l$ for a single training example, where the average is computed over the mini-batch $\mathcal{B}_l$. This is convenient, since this quantity can be computed efficiently using the back-propagation algorithm \cite{rumelhart1986}. Indeed, deep learning frameworks such as PyTorch \cite{paszke2019} compute $\matr{g}_{\theta_l}$ as standard. 
...
\end{split}
\end{align}
Note that these partial derivatives involve no explicit use of $D \times D$ matrices. All other references to $D \times D$ matrices can be removed by writing the partial derivatives in Equations (\ref{eqn:grad_prior_wrt_psi}) and (\ref{eqn:grad_likelihood_wrt_psi}) with respect to $\psi$. Formally,
\begin{equation}
...
Synthetic data was generated in the exact same way as for the experiments in Section \ref{sec:online_fa_posterior_experiments}. Using this data, the true posterior was evaluated according to Equation (\ref{eqn:linear_regression_posterior}) and an approximate posterior with latent dimension $K=1$ was estimated via VIFA. VIFA ran for 5000 epochs with a batch size of $M=100$ and a Monte Carlo average size of $L=10$.  The learning rates $\eta_\matr{c}$,  $\eta_\matr{F}$ and $\eta_\beta$ were set to 0.01, 0.0001 and 0.01, respectively. The reason for using a smaller learning rate for $\matr{F}$ is that its contribution to the full covariance matrix is $\matr{F}\matr{F}^\intercal$. Since this is regression, the average negative log-likelihood term used on line 12 of Algorithm \ref{alg:vi_fa} was set to
...
The work in this thesis focused on improving posterior estimation of neural networks. The overall objective was to use properties of FA to overcome some of the limitations of SWAG and SLANG. More specifically, the two main project goals were: (1) implement two novel online FA algorithms which scale to high-dimensional data, such as the parameter vectors of deep neural networks; (2) implement a model-agnostic VI algorithm which learns a FA posterior of a neural network. 
...
This section completes the derivation of the Bayesian linear regression model from Section \ref{sec:bayesian_lr}. First, applying the logarithm to the prior in Equation (\ref{eqn:linear_regression_prior}),
...
This section presents a full derivation of the partial derivatives given in Equations (\ref{eqn:derivatives_wrt_F}) and (\ref{eqn:derivatives_wrt_Psi}) for the online SGA FA algorithm. Recall from Section \ref{sec:gradient_fa} that the goal is to compute the expected value of the partial derivatives of Equation (\ref{eqn:log_fa_cond_dist}) over the distribution $p(\matr{h}_t | \theta_t, \matr{F}, \Psi)$.
...
\begin{equation}%\label{eqn:em_Psi_update}
	\Psi = \text{diag}\Bigg(\text{diag}\Bigg( \frac{1}{T} \sum_{t=1}^T \matr{d}_t \matr{d}_t^\intercal - 2\matr{FA}^\intercal + \matr{FHF}^\intercal \Bigg)\Bigg).
\end{equation}
Note that this optimisation involves summing over $t=1,\dots,T$. Moreover, on each iteration all components of the sums are updated. As in the E-step, updating all components is not possible in an online algorithm. Therefore, in the online version these sums will be updated incrementally on each iteration. 
...
Figure \ref{fig:fa_wasserstein} shows the scaled 2-Wasserstein distance between the Gaussian distribution defined by each estimated FA model and the Gaussian distribution defined by the true FA model, for the same combinations of observation dimension, latent dimension, spectrum range and sample size as Figure \ref{fig:fa_covar_distance}. Finally, Table \ref{table:fa_covar_distance} and Table \ref{table:fa_wasserstein} contain the numerical results corresponding to the data plotted in Figure \ref{fig:fa_covar_distance} and Figure \ref{fig:fa_wasserstein}, respectively, for the case of 100,000 training samples. 
...
\section{UCI Posterior Estimation with VIFA}\label{app:uci_posterior}

This section provides more details and results for the linear regression posterior estimation experiments using VIFA in Section \ref{sec:vifa_posterior_uci}. Table \ref{table:uci_datasets} shows the number of instances and input variables in each UCI regression dataset. Note that the original Energy Efficiency dataset has two target variables, heating load and cooling load, but in these experiments only the heating load was used. Table \ref{table:vifa_uci_hyperparameters} shows the hyperparameter values used in the experiments.
...
Figures \ref{fig:posterior_energy_efficiency} and \ref{fig:posterior_concrete_strength} show a qualitative comparison of the true posterior of a linear regression model and the approximate posterior learned by VIFA, in the case of the Energy Efficiency and Concrete Compressive Strength datasets, respectively. Recall that the same comparison was given for the Boston Housing and Yacht Hydrodynamics datasets in Section \ref{sec:vifa_posterior_uci}. Finally, Table \ref{table:linear_regression_vi_posterior_uci} contains quantitative results for all four UCI datasets. 
...
This section provides more details about the loss functions used in the neural network prediction experiments in Section \ref{sec:uci_nn_predictions}. While training, the average negative log-likelihood term used on line 12 of Algorithm \ref{alg:vi_fa} was set to
