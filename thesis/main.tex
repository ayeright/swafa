...
%According to \cite{kingma2013}, on each iteration of the algorithm it suffices to compute these estimates using a single $\theta \sim q(\theta)$ and a mini-batch of training data sampled from $\mathcal{D}$. 
...
\begin{algorithm}[!htbp] 
	...
	\label{alg:vi_fa}
	\begin{algorithmic}[1]
		\Require{Dataset $\mathcal{D} = \{(\matr{x}_n, y_n)\}_{n=1}^N$, observation dimension $D$, latent dimension $K$, precision $\alpha > 0$, iterations $T$, mini-batch size $M$, Monte Carlo average size $L$, learning rates $\eta_\matr{c},  \eta_\matr{F}, \eta_\beta > 0$} 
		\State Initialise $\matr{c} = \matr{0}^D$, $\matr{F} \in \R^{D \times K}$, $\psi = \matr{1}^D$
		\State $\beta \leftarrow \log \psi$
		\State Initialise $\hat{\matr{g}}_\matr{c} = \matr{0}^D$, $\hat{\matr{G}}_\matr{F} = \matr{0}^{D \times K}$, $\hat{\matr{g}}_\beta = \matr{0}^D$
		...
		\For {$t = 1,\dots, T$}
			...
			\State $\hat{\matr{g}}_\beta \leftarrow \hat{\matr{g}}_\beta + \matr{s} + \frac{\alpha}{2} \psi + \frac{1}{2} \hat{\matr{g}}_\theta \odot \big(\psi^{1/2} \odot \matr{z}\big)$
			\If {$t \bmod L = 0$}
				...
				\State $\beta \leftarrow \beta - \eta_\beta \big(\frac{1}{L}\hat{\matr{g}}_\beta\big)$
				\State $\psi \leftarrow \exp \beta$
				\State Reset $\hat{\matr{g}}_\matr{c}, \hat{\matr{G}}_\matr{F}$ and $\hat{\matr{g}}_\beta$ according to line 3
