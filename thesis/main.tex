...
\begin{split}
	...
	& =  \Psi^{-1}\matr{F}\big(
	\matr{I} - \Sigma \matr{F}^\intercal \Psi^{-1} \matr{F} - \Sigma
	\big)\matr{F}^\intercal \Psi^{-1} \matr{F} \Sigma \\
	& =  \Psi^{-1}\matr{F}\big(
	\matr{I} - \Sigma (\matr{F}^\intercal \Psi^{-1} \matr{F} + \matr{I})
	\big)\matr{F}^\intercal \Psi^{-1} \matr{F} \Sigma \\
	& =  \Psi^{-1}\matr{F}\big(
	\matr{I} - \Sigma \Sigma^{-1}
	\big)\matr{F}^\intercal \Psi^{-1} \matr{F} \Sigma \\
	& = \matr{0}.
\end{split}
\end{align}
...
\begin{split}
	\matr{s} 
	 ...
	 & = \diag\big(
	 \matr{FF}^{\intercal} + \Psi
	 -2 \matr{F} \Sigma(\matr{F}^\intercal \Psi^{-1} \matr{F} + \matr{I}) \matr{F}^\intercal
	 + \matr{F} \Sigma \matr{F}^\intercal
	 + \matr{F}  \Sigma \matr{F}^\intercal \Psi^{-1} \matr{F} (\matr{F}^{\intercal} \Psi^{-1} \matr{F} + \matr{I}) \Sigma \matr{F}^\intercal
	  \big) \\
	 & = \diag\big(
	 \matr{FF}^{\intercal} + \Psi
	 -2 \matr{F} \Sigma \Sigma^{-1} \matr{F}^\intercal
	 + \matr{F} \Sigma \matr{F}^\intercal
	 + \matr{F}  \Sigma \matr{F}^\intercal \Psi^{-1} \matr{F} \Sigma^{-1} \Sigma \matr{F}^\intercal
	 \big) \\
	 & = \diag\big(
	 \matr{FF}^{\intercal} + \Psi
	 -2 \matr{F} \matr{F}^\intercal
	 + \matr{F} \Sigma \matr{F}^\intercal
	 + \matr{F}  \Sigma \matr{F}^\intercal \Psi^{-1} \matr{F} \matr{F}^\intercal
	 \big) \\
	 & = \diag\big(
	 \Psi - \matr{F} \matr{F}^\intercal
	 + \matr{F} \Sigma (\matr{I} + \matr{F}^\intercal \Psi^{-1} \matr{F}) \matr{F}^\intercal
	 \big) \\
	 & = \diag\big(
	 \Psi - \matr{F} \matr{F}^\intercal
	 + \matr{F} \Sigma \Sigma^{-1} \matr{F}^\intercal
	 \big) \\
	 & = \diag(\Psi).
\end{split}
\end{align}
Hence, substituting into Equation (\ref{eqn:vi_expected_derivatives_wrt_Psi}),
\begin{align}
\begin{split}
	\nabla_\Psi \E_{q(\theta)} [\log q(\theta)] 
	 & = \diag\Bigg(
	 \frac{1}{2} \diag(\Psi^{-2}) \odot \diag(\Psi)
	 \Bigg)  - \frac{1}{2} \Psi^{-1} \\
	 & = \diag\Bigg(
	 \frac{1}{2} \diag(\Psi^{-1})
	 \Bigg)  - \frac{1}{2} \Psi^{-1} \\
...
In order for the algorithm to be practical, explicit use of $D \times D$ matrices must be avoided. As in Algorithm \ref{alg:gradient_fa}, it is far more efficient to work with $\psi = \diag(\Psi)$ rather than $\Psi$ itself. The required partial derivatives with respect to $\psi$ are
\begin{equation}
	\nabla_\psi \E_{q(\theta)} [\log q(\theta)] = \matr{0},
\end{equation}
\begin{equation}
...
\begin{equation}
	\nabla_\beta \E_{q(\theta)} [\log q(\theta)] = \matr{0},
\end{equation}
\begin{equation}
...
\begin{algorithm}[!htbp] 
	...
	\begin{algorithmic}[1]
		...
		\For {$t=1,\dots,T$}
			...
			\State $\matr{g}_\beta \leftarrow \frac{\alpha}{2} \psi + \frac{1}{2} \matr{g}_\theta \odot \big(\psi^{1/2} \odot \matr{z}\big)$
