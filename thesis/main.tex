...
\section{Experiments}\label{sec:online_fa_experiments}
...
\subsection{Estimating the Posterior Online}\label{sec:linear_models_online_posterior}
...
The aim of these experiments is to test how well the online FA algorithms from Section \ref{sec:fa} are able to approximate the true posterior distribution of the weights of a linear regression model. For this purpose, four real regression datasets from the UCI Machine Learning Repository \cite{dua2019} were used. Namely, the Boston Housing\footnote{https://archive.ics.uci.edu/ml/machine-learning-databases/housing}, Concrete Compressive Strength\footnote{https://archive.ics.uci.edu/ml/datasets/concrete+compressive+strength}, Energy Efficiency\footnote{https://archive.ics.uci.edu/ml/datasets/energy+efficiency} and Yacht Hydrodynamics\footnote{http://archive.ics.uci.edu/ml/datasets/yacht+hydrodynamics} datasets. More details about these datasets are given in Table \ref{table:uci_datasets}. Note that the Energy Efficiency dataset has two target variables, heating load and cooling load, but in these experiments only the heating load was used. 
...
In each experiment, a linear regression model (with no bias term) was trained on one of these datasets via mini-batch SGD for 500 epochs, with a batch size of 32 and a constant learning rate of 0.001. The L2 regularisation strength was set to $\lambda = \frac{\alpha}{\beta}$, as justified in Section \ref{sec:linear_models_online_posterior}. Recall that $\beta$ is just the reciprocal of the variance of the target variable and $\alpha$ is a hyperparameter which controls the width of the prior on the linear regression weights. Both $\alpha$ and $\beta$ are used when computing $\matr{A}$ in Equation \ref{eqn:linear_model_A_and_b}. In order for the prior to have a noticeable effect on the posterior, $\alpha$ was set to the maximum diagonal entry of $\beta \sum_{n=1}^N \matr{x}_n \matr{x}_n^\intercal$.

Before training began, two FA models were initialised according to Equations (\ref{eqn:F_init}) and (\ref{eqn:psi_init}) with $\sigma=0.1$. Their observation dimension was set to the number of weights in the linear regression model, as their purpose was to approximate the posterior distribution of the weights. After an initial warm-up period of training the linear model for 10 epochs, the two FA models began to be updated, one via online SGA and the other via online EM (Algorithms \ref{alg:gradient_fa} and \ref{alg:online_em}, respectively). An update to each FA model was performed using the weight vector sampled after each mini-batch update to the linear model. As in the experiments in Section \ref{sec:online_fa_experiments}, $\matr{F}$ and $\Psi$ remained fixed during the first 100 updates to the FA models, with only the running average being updated in this period. The SGA updates were performed with a learning rate of 0.001. 

The mean and covariance of the true posterior of the linear regression weights, for the specific values of $\alpha$ and $\beta$, were computed prior to training according to Equation \ref{eqn:linear_regression_posterior_params}. After each set of 10 training epochs, the distance between the parameters of the true posterior and the posteriors estimated by the online FA algorithms was computed. For each dataset, this experiment was run with the latent dimension of the FA models set to each value in $\{1, 2, \dots, d-1\}$, where $d$ is the number of weights (or equivalently, the number of input variables in the dataset). Each experiment was repeated ten times. In each trial a different random seed was used for initialising the weights of the linear model and also initialising the parameters of the FA models.
...
%	\centering
%	\begin{tabular}{cc}
%		\includegraphics[width=100mm]{plots/linear_models_covariance_distance__boston_housing} \\
%		\includegraphics[width=100mm]{plots/linear_models_covariance_distance__concrete_strength} \\
%		\includegraphics[width=100mm]{plots/linear_models_covariance_distance__energy_efficiency} \\
%		\includegraphics[width=100mm]{plots/linear_models_covariance_distance__yacht_hydrodynamics} \\
%	\end{tabular}
%	\caption{For four difference UCI regression datasets, this figure show how well the online FA algorithms are able to learn the covariance matrix of the posterior distribution of the weights of a linear regression model trained on the dataset. Results for online SGA and online EM are show in the left and right axes, respectively. Each plot shows the Frobenius norm of the difference between the true covariance matrix and the estimated covariance matrix divided by the Frobenius norm of the true covariance matrix. Each line corresponds to a different latent dimension for the learned FA models. Each data point shows the mean value over ten trials with different random seeds, and standard error bars are also plotted.}
%	\label{fig:linear_models_covariance_distance}
%\end{figure}
