...
def test_get_features_and_targets(n_samples, n_features):
    ...
    assert np.isclose((X.numpy() * stds) + means, features).all()
...
def test_compute_true_covar(n_samples, n_features, alpha, beta):
    X = torch.randn(n_samples, n_features)

    actual_S, actual_alpha = compute_true_posterior_covar(X, beta, alpha=alpha)
    ...
    if alpha is None:
        assert actual_alpha == torch.diag(B).max()
    else:
...
@pytest.mark.parametrize("alpha", [None, 0.1])
@pytest.mark.parametrize("beta", [0.01, 0.1])
def test_compute_true_posterior_mean(n_samples, n_features, alpha, beta):
    X = torch.randn(n_samples, n_features)
    y = torch.randn(n_samples)

    S, _ = compute_true_posterior_covar(X, beta)
...
def test_compute_true_posterior(n_samples, n_features, alpha, beta):
    X = torch.randn(n_samples, n_features)
    y = torch.randn(n_samples)

    actual_m, actual_S, actual_alpha, actual_beta = compute_true_posterior(X, y, alpha, beta)
    ...
    expected_S, expected_alpha = compute_true_posterior_covar(X, actual_beta, alpha=alpha)
    assert torch.isclose(actual_S, expected_S).all()
